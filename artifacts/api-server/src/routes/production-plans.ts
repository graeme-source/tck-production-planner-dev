...
import { buildPlanSchedule } from "../lib/day-schedule";
...
// so any issue with that module can never take down server startup or
// the rest of the router.
import type { ProductionPlanPdfData } from "../pdf/production-plan-pdf";
...
router.get("/:id/lock-pdf", async (req, res) => {
  ...
  try {
    const { renderProductionPlanPdf } = await import("../pdf/production-plan-pdf.js");
...
// Moved to lib/day-schedule.ts (charter rule 4, 2026-09-25); this is the call site only.
router.get("/:id/schedule", async (req, res) => {
  const planId = Number(req.params.id);
  if (!Number.isFinite(planId)) { res.status(400).json({ error: "Invalid plan id" }); return; }
  const schedule = await buildPlanSchedule(planId);
  if (!schedule) { res.status(404).json({ error: "Plan not found" }); return; }
  res.json(schedule);
});
