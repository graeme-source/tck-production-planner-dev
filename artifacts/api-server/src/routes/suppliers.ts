...
router.post("/", validate(CreateSupplierBody), async (req, res) => {
  const { name, contactName, email, phone, orderingPhone, website, address, notes, orderFrequency, orderDays, leadTimeDays, cutoffTime } = req.body;
  const [row] = await db.insert(suppliersTable).values({ name, contactName, email, phone, orderingPhone: orderingPhone || null, website, address, notes, orderFrequency: orderFrequency ?? "daily", orderDays: orderDays || null, leadTimeDays: leadTimeDays ?? 1, cutoffTime: cutoffTime || "17:00" }).returning();
